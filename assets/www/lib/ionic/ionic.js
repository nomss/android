    ...
    if ((!ionic.tap.isTextInput(e.target) || tapLastTouchTarget !== e.target) && !(/^(select|option)$/i).test(e.target.tagName)) {
      ...
      // The focusin event handles the chance of focus changing after the keyboard shows.
    	if(/MSIE 10/i.test(navigator.userAgent) || /rv:11.0/i.test(navigator.userAgent)) {
			// Don't prevent default in IE - TMERF-328: Enable select/copy text
    	} else {
    		e.preventDefault();
    	}
    }
...
function keyboardFocusIn(e) {
  ...
  // Mobile team fix - Preventing page shift when input gets focus on Desktop (IE)
  if (window.isCordova) {
  ...
  initialize: function(options) {
    ...
    self.options = {
      ...
      getContentWidth: function() {
    	  // Mobile team - image scroller fix
    	  if (self.options.scrollingX) {
    			  return Math.max(self.__content.scrollWidth - self.__content.offsetWidth, (self.__content.offsetWidth - self.__content.scrollWidth));
    	  } else {
